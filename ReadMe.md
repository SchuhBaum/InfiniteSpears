...
1. Download the file  `InfiniteSpears.zip` from [Releases](https://github.com/SchuhBaum/InfiniteSpears/releases/tag/v2.3.3).
...
#### (Rain World v1.11)
v2.3.3:
- Fixed a bug where poison spears would not be duplicated correctly.

v2.3.2:
- (pick up on release) Added this option (disabled by default). When enabled,
  backspears have priority. You only pick up items from the ground when
  releasing the grab key.
